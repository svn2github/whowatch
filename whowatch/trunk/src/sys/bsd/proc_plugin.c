...
static struct netconn_t *validate(unsigned int inode, char *s)
{
	...
	int i, offset = 2 * sizeof(struct list_head);
...
//	t.used = tmp->used = 0;
	if(!memcmp(&t + offset, tmp + offset, sizeof(t) - offset)) {
//dolog(__FUNCTION__ ": %d %d %d found, not changed\n", inode, t.s_port, t.d_port);	
		return tmp; 
	}
//dolog(__FUNCTION__ ": %d %d %d found,changed\n", inode, t.s_port, t.d_port);	
	tmp->s_addr = t.s_addr;	
	tmp->s_addr = t.s_addr;	
	tmp->d_addr = t.d_addr;	
	tmp->s_port = t.s_port;	
...
print(buf);
if(t->d_addr) {
//	snprintf(buf, sizeof buf, " -> %s:%d", inet_ntoa(*((struct in_addr*)&t->d_addr)),
//		t->d_port);
//	print("%s", buf);
	print(" -> %s:%d", inet_ntoa(*((struct in_addr*)&t->d_addr)));
}
